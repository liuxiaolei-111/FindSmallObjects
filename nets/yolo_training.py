...
from .focalloss import VarifocalLoss
...
class YOLOLoss(nn.Module):
    #for backbone test
    # def __init__(self, num_classes, strides=[4, 8,16]):
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        # loss_cls    = (self.focal_loss_from_logits(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)).sum()
        reg_weight  = 5.0
    ...
    def get_in_boxes_info(self, gt_bboxes_per_image, expanded_strides, x_shifts, y_shifts, total_num_anchors, num_gt, center_radius = 5):
        #-------------------------------------------------------#
